...
def cmd_pose_vpd(args: argparse.Namespace) -> int:
    ...
    try:
        from mmd_trace.single_image import generate_vpd_from_image
        import numpy as np

        axis_matrix = None
        if args.axis_matrix:
            values = [float(v) for v in args.axis_matrix.split(",")]
            if len(values) != 9:
                raise ValueError("--axis_matrix must be 9 comma-separated floats")
            axis_matrix = np.array(values, dtype=float).reshape(3, 3)
...
def cmd_debug_visualize(args: argparse.Namespace) -> int:
    """Handle debug-visualize command."""
    try:
        from mmd_trace.debug_pose_visualizer import generate_debug_visualization
        import numpy as np
        ...
        output_files = generate_debug_visualization(
            image_path=args.image,
            pmx_path=args.pmx,
            out_dir=args.out,
            task_model=args.task_model,
        ...
        print(f"\nDebug visualization generated successfully!")
        ...
        LOG.error("Error generating debug visualization: %s", e)
        import traceback
        traceback.print_exc()
        return 1
...
def main() -> int:
    ...
    # debug-visualize command
    debug_parser = subparsers.add_parser(
        "debug-visualize",
        help="Generate debug visualization with bone centers and axes",
    )
    ...
    debug_parser.add_argument(
        ...
        dest="show_fingers",
        action="store_false",
        help="Hide finger bones in visualization",
    )
    debug_parser.set_defaults(show_fingers=True)
    debug_parser.add_argument(
        "--axis_scale",
        type=float,
        default=1.0,
